/* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+aspnet+bash+c+csharp+cpp+coffeescript+css-extras+git+go+http+java+less+markdown+objectivec+perl+php+php-extras+python+jsx+ruby+scss+sql+swift&plugins=line-numbers+autolinker */
...
Prism.hooks.add("after-highlight", function (e) {
    ...
    if (t && /pre/i.test(t.nodeName) && -1 !== t.className.indexOf("line-numbers")) {
        var n, a = 1 + e.code.split("\n").length, r = new Array(a);
        r = r.join("<span></span>"), n = document.createElement("span"), n.className = "line-numbers-rows", n.innerHTML = r, t.hasAttribute("data-start") && (t.style.counterReset = "linenumber " + (parseInt(t.getAttribute("data-start"), 10) - 1)), e.element.appendChild(n)
    }
});
;
!function () {
